...
public class JsonSetCoursier implements IonHandler {


    public JsonSetCoursier(String link , final Context ctx)
    {
        Ion.with(ctx)
                .load(link)
                .asString()
    ...
    public void Main_Function(String result)
    {
        ...
        try
        {
            if(Integer.parseInt(result_failed) == 406)
            {
                ...
                CoursierRepository.list_coursier.get(CoursierRepository.list_coursier.size()-1).id = Integer.parseInt(result_failed);
