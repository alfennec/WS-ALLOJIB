package com.fennec.allojib.config;

public class constant {

    //public static  String url_host = "http://192.168.1.5/livraison/";
    //public static  String url_host = "http://allojib.eradroids.com/";
    public static  String url_host = "https://allojib.ma/";

    ////localhost/livraison/json/getOrderPlat.php?id_order=1
    ////localhost/livraison/json/getPassOrderPlat.php?id_client=1
    ////localhost/livraison/json/updateClient.php?id=17&email=wissal@gmail.com&pass=123456&nom=Berrahal&prenom=Wissal&tel=0645154875&adresse=haylaymoune&ville=oujda&sexe=1

    ////localhost/livraison/json/setCoursier.php?id_client=17&adr_col=nour&adr_liv=kods&detail=rien&date_col=12&heure_col=13&tel=0615442168&situation=1

}
